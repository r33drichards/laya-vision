...
| `prepare.py`: fixed constants, data and `evaluate_bpb` | `harness.py` | The budget is fixed but 15 minutes, not 5: at 5 the games objective was starved (tag `sep23-games`, `program.md`). The metric is our eval suite (34 sets, sampled), plus parameter count and L4 latency. Runs on Modal: an H100 trains, an L4 times. |
