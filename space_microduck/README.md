...
MuJoCo physics with upstream's Microduck model walking on upstream's trained policy (rendered through OSMesa,
`packages.txt`), and `microduck:sim2d`, the flat cartoon. Each step the model sees the duck's camera and picks
