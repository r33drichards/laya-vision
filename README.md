...
- **Yoga postures** (`laya/yoga.py`, `prepare_yoga`): the Kaggle [yoga posture dataset](https://www.kaggle.com/datasets/tr1gg3rtrash/yoga-posture-dataset), 47 poses. Each image is a `choice` over its pose and 11 random others, written "English (Sanskrit)", e.g. "Reverse Warrior (Parsva Virabhadrasana)". Duplicate files, and files filed under two poses, are dropped. That leaves 2,330 train and 386 val records, split by content hash. Add `yoga` to `--datasets` to train on it.
...
modal run modal_app.py::prepare_yoga                                      # -> /data/vqa/yoga_poses
