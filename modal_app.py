...
def _traced(*attrs):
    ...
    attributes, stdout as logs and numeric results as metrics, sent to ``laya.telemetry.DEFAULT_ENDPOINT``. laya
    is imported in the container, not by the client (it needs torch)."""
    def deco(fn):
        @functools.wraps(fn)
